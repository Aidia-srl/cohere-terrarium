...
export class PyodidePythonEnvironment implements PythonEnvironment {
        ...
        document: {
          getElementById: (id: any) => {
            if (id.includes("canvas"))
              return null; // lol don't ask ... this is needed! https://github.com/pyodide/matplotlib-pyodide/blob/61935f72718c0754a9b94e1569a685ad3c50ae91/matplotlib_pyodide/wasm_backend.py#L48
            else
              return {
                addEventListener: () => {},
                style: {},
  ...
  listFilesRecursive(dir: string) {
    ...
    if (!entries) {
      console.warn(`No entries found in directory: ${dir}`);
      return files;
    }
  ...
  async runCode(code: string, files: any[]): Promise<CodeExecutionResponse> {
    ...
    try {
      ...
      let new_files = allFiles
        .filter(
          (f) =>
            !input_file_names.includes(
              f.slice(pythonEnvironmentHomeDir.length + 1)
            )
        )
        .map((f) => {
          return {
      ...
      console.log(
        ...
        result_reporting,
        "stdout:",
        this.out_string.replace(/\n/g, "\\n"),
        "stderr:",
        this.err_string.replace(/\n/g, "\\n")
      );
